...
public class FileUpDownUtil {
                ...
                while(params.hasNext()) {
                    try {
                        ...
                        if (item.isFormField()) { // 텍스트 데이터라면
                            ...
                            String name = item.getFieldName();
                            // 업로드한 파일정보를 디비에 저장할때 원본파일명 그대로 저장되게 함.
    ...
    public String FileUpload2(MultipartFile mf, String uuid) {
        String ofname = mf.getOriginalFilename(); // 파일 원본명 가져옴

        // abc.txt                : fnames[1] => txt
        // abc.123.xyz.png : fnames[3] => png
//        String fnames[] = ofname.split("[.]");
//        String ftype = fnames[fnames.length - 1]; // 확장자 추출
